...
Select Client.Name, Package.Contents, Package.Weight
FROM Client 
JOIN Package ON Package.Recipient = Client.AccountNumber
WHERE Package.Weight = 1.5;

-- 7.2 What is the total weight of all the packages that he sent?
SELECT TOTAL(Package.Weight), AVG(Package.Weight), COUNT(Package.Weight) as Total_Packages
FROM Package
WHERE Package.Sender IN (SELECT Client.AccountNumber
	FROM Client 
	JOIN Package ON Package.Recipient = Client.AccountNumber
	WHERE Package.Weight = 1.5);
